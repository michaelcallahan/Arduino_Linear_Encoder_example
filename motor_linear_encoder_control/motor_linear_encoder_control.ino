...
float readEncoder() {
  ...
  for (int i = 0; i < 13; i++) {
    ...
    for (int bit = 0; bit < 4; bit++) {
      while (digitalRead(CK_PIN) == LOW); // Hold until clock is high
      while (digitalRead(CK_PIN) == HIGH); // Hold until clock is low
  ...
  switch (decimal) {
    ...
    default: dpp = 1.0; break;  // Default to 1.0 if unexpected decimal place
  }
